...
def main():
    ...
    store_rows = []
    day_store_ids_from_store_rows = defaultdict(set)
    for (_d, _sid), a in sorted(store_agg.items(), key=lambda x: (x[0][0], x[0][1])):
        ...
        if abs(total) < 1e-12:
            continue
        day_store_ids_from_store_rows[a["delivery_date"]].add(a["store_id"])
        store_rows.append(
            {
    ...
    # build task rows after store rows so store_count matches distinct store_id in store detail
    task_rows = []
        ...
        task_rows.append(
            {
                ...
                "store_count": len(day_store_ids_from_store_rows[d]),
