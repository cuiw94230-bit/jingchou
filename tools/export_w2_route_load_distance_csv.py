...
OUTPUT_CSV = PROJECT_ROOT / "tools" / "w2_route_load_distance.csv"
DC_ID = "DC"
...
def main():
    ...
    for r in rows:
        ...
        if not route_id_raw.startswith("2"):
            continue
    ...
    out = []
    for (delivery_date, route_id), stop_map in sorted(route_stops.items(), key=lambda x: (x[0][0], int(float(x[0][1])))):
        ...
        out.append(
            {
                ...
                "route_load_w2": round(route_load_w2, 6),
