"""构造式求解模块：先给元启发式生成起点，也支持直接输出构造式方案。"""`n`nimport time
...
def solve_constructive(payload, algorithm_key):
    ...
    # 候选门店来源统一使用 payload.stores，避免依赖 base_state.routes 造成空跑
    ...
    for index, store_id in enumerate(ordered_ids):
        if algorithm_key == "vrptw":
            inserted = vrptw_insert_missing(current, [store_id], stores, dist, wave, scenario, vehicle_limit=max(1, len(current)))
        else:
            diag_rows = []
        ...
        if algorithm_key != "savings":
            trace_log.append({
                ...
                "costBreakdown": {k: round(float(v or 0.0), 2) for k, v in ((evaluated.get("components") or {}).items())},
                "textZh": "",
                "textJa": "",
            })
    ...
    if best_cost is None:
        best_cost = evaluate_state(current, stores, dist, wave, scenario) or evaluate_state(base_state, stores, dist, wave, scenario) or 0.0
...
def generate_diverse_initial_state(payload, algorithm_key, rng):
    ...
    # 候选门店来源统一使用 payload.stores，避免依赖初始 routes
    all_store_ids = sorted(stores.keys(), key=lambda sid: str(sid))
    if not all_store_ids:
        return base_state
